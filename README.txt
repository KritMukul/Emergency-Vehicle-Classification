...
An audio classification system to detect emergency vehicle sirens (ambulances, 
fire trucks, police) from regular traffic sounds using machine learning.

DATASET

Total Files: ~9,988 audio samples
Sources:
  1. data/audio/     - Urban Sound 8K dataset (Class 8 = Emergency vehicles)
  2. data/Dataset/   - Custom emergency vehicle recordings (ambulance/firetruck/police)
  3. data/traffic/   - Regular traffic sounds
...
- Energy-based segmentation: Extracts the loudest 3-second window
  (ensures emergency sirens are captured)
...
Three models achieve high accuracy in distinguishing emergency vehicle sirens
from regular traffic sounds:
...
Issue: "FileNotFoundError: data/audio/"
Fix: Ensure data folders exist and contain .wav files

Issue: Audio files not processing
Fix: Check that files are in .wav format

Issue: Low accuracy
Fix: Ensure balanced dataset with sufficient examples of both classes
